...
import { createDreamiPlacer, createZonePicker } from "./zones.mjs";
...
export async function pullAgents({
  ...
  const boormis = boormiRows.map((row, i) => {
    ...
    return {
      ...
      email: row.EMAIL,
      password,
      zone: zone?.name ?? "서울 강남",
      order: orderPayload(i, zone),
    };
  });
