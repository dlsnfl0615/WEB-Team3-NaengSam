...
import { createDreamiPlacer, createZonePicker, describePlacement, resolveCenter } from "./zones.mjs";
...
/**
 * 전국 모드에서 쓸 주소 목록. 도로명은 위 강남 주소를 재활용하되 앞에 도시명을 붙인다.
 *
 * 백엔드 DevCoordinatesService(kakao.dev-zone-mode=national)가 **주소의 첫 토큰**으로 격자를 놓을 도시를
 * 고르므로, 한 주문의 출발지와 도착지가 같은 도시명으로 시작하면 같은 도시 안에 떨어진다. 도시를 가로지르는
 * 수백 km짜리 배달(요금 수십만 원)이 생기지 않게 하려는 것이다.
 *
 * 도시 목록은 NATIONAL_ZONES와 같아야 한다 — 드리미가 그 도시들에 뿌려지기 때문이다.
 */
const CITY_STREETS = ROAD_ADDRESSES.map((address) => address.split(" ").slice(1).join(" "));

function nationalAddress(zone, streetIndex) {
  return `${zone.city} ${CITY_STREETS[streetIndex % CITY_STREETS.length]}`;
}

/**
 * 부르미 i번이 쓸 주문 payload. 출발지와 도착지가 같아지지 않게 어긋나게 집는다.
 * `zone`을 주면 그 도시 안의 주소로 만든다(전국 모드).
 */
export function orderPayload(i, zone = null) {
  const origin = zone ? nationalAddress(zone, i) : ROAD_ADDRESSES[i % ROAD_ADDRESSES.length];
  const destination = zone
    ? nationalAddress(zone, i + 1 + (i % 3))
    : ROAD_ADDRESSES[(i + 1 + (i % 3)) % ROAD_ADDRESSES.length];
  return {
    ...
    destinationAddressLine1:
      destination !== origin
        ? destination
        : zone
          ? nationalAddress(zone, i + 5)
          : ROAD_ADDRESSES[(i + 5) % ROAD_ADDRESSES.length],
...
export function buildSeed({ dreamiCount, boormiCount, browserUsers, pointAmount = 10_000_000, center }) {
  ...
  // 드리미 좌표와 부르미 주문 존. 같은 라운드로빈을 쓰므로 존마다 수요·공급 비율이 같다.
  const placeDreami = createDreamiPlacer({ center, mode: center.mode });
  const pickZone = createZonePicker({ mode: center.mode });
  ...
  const dreamis = Array.from({ length: dreamiCount }, (_, i) => {
    ...
    const { lat, lng, zone } = placeDreami(i);
    return { boormiId, email, password: SEED_PASSWORD, lat, lng, zone };
  });
  ...
  const boormis = Array.from({ length: boormiCount }, (_, i) => {
    ...
    const zone = center.mode === "national" ? pickZone(i) : null;
    return { boormiId, email, password: SEED_PASSWORD, zone: zone?.name ?? "서울 강남", order: orderPayload(i, zone) };
  });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  console.error(
    `${sqlOut}: 계정 ${browserUsers.length + agents.dreamis.length + agents.boormis.length}개\n` +
      ...
      `드리미 분포: ${describePlacement(agents.dreamis)}\n` +
