...
const { resolveCenter, describePlacement, NATIONAL_ZONES } = await import("./modules/zones.mjs");
...
async function seedUsers() {
  ...
  if (config.useExistingAccounts) {
    ...
    line(`배치  ${describePlacement(agents.dreamis)}`);
...
async function checkKakaoStub(agents) {
  ...
  await checkZoneMode(call, probe);
}

/**
 * 백엔드 좌표 스텁이 전국 모드(`KAKAO_DEV_ZONE_MODE=national`)로 떠 있는지 확인한다.
 *
...
async function checkZoneMode(call, probe) {
  const center = resolveCenter();
  if (center.mode !== "national") {
    item("분포", "주문 존 집중 (DIST_MODE=local)", "✓ 원본 matchingtest와 동일");
    return;
  }

  const res = await call(probe, "POST", "/api/v1/address/place", {
    origin: "서울 강남구 테헤란로 152",
    originDetail: "1층",
