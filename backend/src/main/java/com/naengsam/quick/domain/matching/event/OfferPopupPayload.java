 ...
 * offeredAt/expiresAt은 서버의 zone 정보가 없는 LocalDateTime이 아니라 Instant로 내려준다 — zone이 없는 시각 문자열을
 * 프론트가 브라우저 로컬 시간대로 해석하면(서버 시간대와 다를 경우) expiresAt이 이미 지난 것처럼 보여 즉시 만료로 표시되는 문제가 있었다.
 */
public record OfferPopupPayload(
        ...
        String imageKey,
        Instant offeredAt,                // 제안이 생성된 시각
        Instant expiresAt                 // 응답 마감 시각(offeredAt + ttl)
) {
