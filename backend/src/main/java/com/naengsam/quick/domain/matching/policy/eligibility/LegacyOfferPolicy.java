 ...
 * 거절했던(BOORMI_REJECTED) 조합은 다시 후보로 삼지 않는다. 타의로 회수됐거나(WITHDRAWN) 부르미 응답 timeout(BOORMI_EXPIRED)인 경우는 드리미 본인의 잘못이 아니므로 다시
 * 후보로 허용한다. 이전 이력이 없으면 당연히 허용한다.
 * <p>evaluatedAt은 시각을 실제로 사용하지 않지만, {@link MatchingEligibilityPolicy} 계약에 맞추기 위해
 * 받기만 한다.
 */
public class LegacyOfferPolicy implements MatchingEligibilityPolicy {
