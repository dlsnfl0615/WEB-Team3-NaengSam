...
public class NearbyOrderFinder {
    ...
    /**
     * 기준 좌표에서 반경(m) 이내에 있는 대기중인 주문을 가까운 순으로 조회한다. 대기중인 주문은 별도 등록 큐가 없고, 매칭 시작 후(OPEN) 아직 확정되지 않은 주문에서
     * {@link MatchingService#waitingOrders()}로 도출된다.
     *
    ...
    public List<NearbyOrderDto> find(NearbyOrderRequest request) {
        ...
        return matchingService.waitingOrders().stream()
                .map(order -> NearbyOrderDto.from(order,
                        geoDistanceCalculator.distanceMeters(origin, order.location())))
