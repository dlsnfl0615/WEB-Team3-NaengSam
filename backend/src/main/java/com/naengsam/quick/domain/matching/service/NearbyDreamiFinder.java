...
public class NearbyDreamiFinder {
    ...
    /**
     * 기준 좌표에서 반경(m) 이내에 있는 대기중인 드리미를 가까운 순으로 조회한다. 대기중인 드리미는
     * {@link MatchingService#waitingDreamis()}에서 가져온다.
     *
    ...
    public List<NearbyDreamiDto> find(NearbyDreamiRequest request) {
        ...
        return matchingService.waitingDreamis().stream()
                .map(dreami -> NearbyDreamiDto.from(dreami, boormiService.distanceMeters(origin, dreami.location())))
