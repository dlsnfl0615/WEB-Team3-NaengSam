...
/**
 * 드리미 제안 수락/거절 엔드포인트가 서비스에 위임하는지 검증한다. 소유권 체크·DB 반영 등 실제 로직은 DreamiServiceTest에서 다룬다.
 */
class DreamiControllerTest {
    ...
    @Test
    void 드리미_수락_요청을_서비스에_위임한다() throws Exception {
        ...
        verify(dreamiService).acceptOffer(offerId, dreamiId);
    }

    @Test
    void 드리미_거절_요청을_서비스에_위임한다() throws Exception {
        UUID offerId = UUID.randomUUID();
        UUID dreamiId = UUID.randomUUID();
        ...
        verify(dreamiService).rejectOffer(offerId, dreamiId);
    }
}
